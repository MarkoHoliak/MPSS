...
def rk4(a, L, T, N, h, phi1, phi2, phi_y):
    ...
    def temperature_derivative(temp_array):
        d_temp = np.zeros_like(temp_array)
        for i in range(1, N):
            d_temp[i] = a * (temp_array[i + 1] - 2 * temp_array[i] + temp_array[i - 1]) / dy ** 2
        return d_temp

    # Чисельне рішення методом Рунге-Кутта 4-го порядку
    for t in range(M):
        # Граничні умови на кожному кроці
        temperature[t][0] = phi1
        temperature[t][-1] = phi2
    ...
    # Граничні умови для останнього шару
    temperature[M][0] = phi1
    temperature[M][-1] = phi2
    ...
    for t_idx in range(len(t_values)):
        for y_idx in range(len(y_values)):
            ...
            for k in range(1, terms + 1):
                exp_term = -a * (np.pi * k / L * alpha) ** 2 * t_values[t_idx]
...
def main():
    # дані
    a = 0.34e-6  # Коефіцієнт теплопровідності
    L = 0.01  # Глибина стінки
    T = 60  # Час симуляції
    N = 100  # Кількість вузлів по глибині
    h = 0.01  # Часовий крок
    M = (T/h)-1
    alpha = 3  # Ліва гранична умова
    beta = 22  # Права гранична умова
